...
(defn build-environment
  "Build an immutable descriptor environment from a collection of values."
  [specs]
  (let [descriptors (mapv normalize specs)
...
(defn environment-descriptor [environment id]
  (get-in environment [:descriptors id]))

(defn environment-descriptors [environment]
