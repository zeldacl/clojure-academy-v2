(ns cn.li.fabric1201.client.key-mapping-adapter
  "Vanilla KeyMapping registration for :alternative scheme inputs (Fabric).
   ...
   Fabric has no conflict-context constructor (that is a Forge extension), so
   mappings are created with the plain vanilla constructor and registered with
   Minecraft via fabric KeyBindingHelper in client/init.clj. The live binding
   is read through the public saveString() round-trip — no reflection."
...
;; Fabric does not auto-register on construction — client/init.clj registers
;; each mapping through fabric KeyBindingHelper after bootstrap.
  ...
  (try
    (let [key (key-code->input-key key-code)
          key-mapping (KeyMapping. translation-key
                                   (.getType key) (.getValue key) category)]
...
(defn get-key-code
  ...
  [input-id]
  (when-let [^KeyMapping km (get-key-mapping input-id)]
    (Integer/valueOf (KeyMappingAccess/acKeyCode km))))
