      ...
      (when-let [^LocalPlayer player (.player mc)]
        (let [camera (.getMainCamera (.gameRenderer mc))
          ...
          (let [frame-context (geometry/presentation-frame-context player cam-pos tick)
                ...
                ;; presentation_hud_renderer.clj): extract! memoizes per real
                ;; frame regardless of which stage asks first, so a mismatched
                ;; size here would bleed into the HUD's cached layout.
