(ns cn.li.neoforge1211.platform.init
  "Forge platform initializer.

  Uses shared mc1211 installer with a Forge-specific adapter implementation."
...
(defn- resolve-binding! [binding-name]
  (require 'cn.li.neoforge1211.platform.bindings)
  (or (ns-resolve 'cn.li.neoforge1211.platform.bindings binding-name)
      (throw (ex-info "Missing Forge platform binding"
                      {:binding binding-name
                       :available (->> (ns-publics 'cn.li.neoforge1211.platform.bindings)
                                       keys
                                       sort
                                       vec)}))))
