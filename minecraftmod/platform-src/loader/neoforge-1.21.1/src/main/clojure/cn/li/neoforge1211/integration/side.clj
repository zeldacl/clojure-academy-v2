(ns cn.li.neoforge1211.integration.side
  "Runtime side detection for NeoForge 1.21.1.
...
(defn require-client-ns
  ...
  Args:
    ns-sym: Symbol - Namespace to require (e.g., 'cn.li.neoforge1211.client.init)
...
(defn resolve-client-fn
  ...
  Args:
    var-sym: Symbol - Fully-qualified var symbol (e.g. 'cn.li.neoforge1211.client.init/init-client)

  Returns:
    The resolved fn if on client, nil if on server or if resolution fails.

  Example:
    (when-let [init! (resolve-client-fn 'cn.li.neoforge1211.client.init/init-client)]
      (init!))"
  [var-sym]
  (when (client-side?)
    (try
