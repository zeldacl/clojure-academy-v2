(ns cn.li.neoforge1211.integration.forge-energy
  ...
  (:import [cn.li.neoforge1211.capability CapabilityRegistry ForgeEnergyAdapter]
           ...
           [net.neoforged.neoforge.capabilities Capabilities Capabilities$EnergyStorage]
...
(defn- get-forge-energy-capability
  ...
  (try
    ...
    (catch Exception e
      (log/stacktrace "Error creating Forge Energy capability:" e)
      nil)))
...
(defn register-forge-energy-capability!
  ...
  (let [descriptors (vec (forge-energy-descriptors))]
    (doseq [capability-key (distinct (map target-capability-key descriptors))]
      (CapabilityRegistry/registerBlock (name capability-key) Capabilities$EnergyStorage/BLOCK)
