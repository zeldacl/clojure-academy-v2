(ns cn.li.fabric1211.integration.side
  "Runtime side detection for Fabric 1.21.1."
...
(defmacro when-client
  [& body]
  `(when (cn.li.fabric1211.integration.side/client-side?)
     ~@body))

(defmacro when-server
  [& body]
  `(when (cn.li.fabric1211.integration.side/server-side?)
     ~@body))
