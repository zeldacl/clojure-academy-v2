(ns cn.li.neoforge262.platform.init
  ...
  Uses shared mc262 installer with a Forge-specific adapter implementation."
...
(defn- resolve-binding! [binding-name]
  (require 'cn.li.neoforge262.platform.bindings)
  (or (ns-resolve 'cn.li.neoforge262.platform.bindings binding-name)
      (throw (ex-info "Missing Forge platform binding"
                      {:binding binding-name
                       :available (->> (ns-publics 'cn.li.neoforge262.platform.bindings)
                                       keys
                                       sort
                                       vec)}))))
