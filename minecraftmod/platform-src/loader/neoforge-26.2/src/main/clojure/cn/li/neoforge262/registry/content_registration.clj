(ns cn.li.neoforge262.registry.content-registration
  "NeoForge content registration — thin DeferredRegister callbacks over
  ...
  (:require [cn.li.neoforge262.integration.bootstrap :as bootstrap]
            ...
            [cn.li.mc262.block.logic-pipeline :as logic-pipeline]
            [cn.li.mc262.entity.mob-logic-pipeline :as mob-pipeline]
            [cn.li.mcmod.protocol.metadata :as registry-metadata]
            [cn.li.mc262.block.blockstate-properties :as blockstate-props]
            [cn.li.platform.registry.content-registration-core :as core])
  (:import [cn.li.neoforge262.entity ModEntities]
           [cn.li.mc262.block IScriptedBlock]
           [cn.li.mc262.effect ScriptedMobEffect]
                                   ...
                                   (get [_]
                                     (bootstrap/create-source-fluid
                                       (bootstrap/create-flowing-fluid-properties
                                         (reify java.util.function.Supplier
                                           (get [_] (.get ^DeferredHolder fluid-type-ro)))
                                         (reify java.util.function.Supplier
                                           (get [_] (.get ^DeferredHolder @source-holder)))
                                         (reify java.util.function.Supplier
                                           (get [_] (.get ^DeferredHolder @flowing-holder)))
                                         (when (:has-bucket? block-spec)
                                           (reify java.util.function.Supplier
                                             (get [_] (.get ^DeferredHolder @bucket-holder))))
                                         (when-let [block-id (:block-id block-spec)]
                                           (reify java.util.function.Supplier
                                             (get [_]
                                               (.get ^DeferredHolder (registry-state/get-registered-block-ro block-id)))))
      ...
      (fn [{:keys [entity-id registry-name entity-kind kind-fields category
        ...
        (let [registered-obj (ModEntities/register
          ...
          (when (and (= :scripted-mob entity-kind)
                     (some? registered-obj)
                     (.isBound ^DeferredHolder registered-obj)
                     (get mob-bundles entity-id))
            (mob-pipeline/install-mob-bundle!
              (.get ^DeferredHolder registered-obj)
              (get mob-bundles entity-id)))
                                      ...
                                      (reify java.util.function.Supplier
                                        (get [_]
                                          (SoundEvent/createVariableRangeEvent
                                            (ResourceLocations/of mod-id registry-name)))))]
