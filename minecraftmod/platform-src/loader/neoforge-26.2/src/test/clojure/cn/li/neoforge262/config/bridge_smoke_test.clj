...
(deftest ability-domain-builds-forge-external-file-contract
  (testing "ability domain maps to the player-facing Forge TOML file"
    (let [domain->file-name @(ns-resolve 'cn.li.neoforge262.config.bridge 'domain->file-name)
          build-domain-spec @(ns-resolve 'cn.li.neoforge262.config.bridge 'build-domain-spec)
...
(deftest config-storage-is-initialized
  (testing "private registered-config storage defaults to a map"
    (let [registered-configs-var (ns-resolve 'cn.li.neoforge262.config.bridge 'registered-configs)]
