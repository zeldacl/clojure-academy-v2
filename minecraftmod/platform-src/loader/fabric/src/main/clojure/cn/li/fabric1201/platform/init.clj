...
(def ^:private fabric-adapter
  {:class-access
   ...
   {:player-level (fn [_adapter player] (RuntimeAccessShared/getEntityLevel player))
    ...
    :spawn-tracked-entity-by-id! (fn [_adapter player entity-id speed life] (ParticleEntityShared/spawnTrackedEntityByIdFromPlayer player (str entity-id) (float (or speed 0.0)) (when life (int life))))
