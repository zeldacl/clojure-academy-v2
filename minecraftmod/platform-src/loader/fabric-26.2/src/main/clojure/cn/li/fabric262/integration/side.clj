(ns cn.li.fabric262.integration.side
  "Runtime side detection for Fabric 26.2."
...
(defmacro when-client
  [& body]
  `(when (cn.li.fabric262.integration.side/client-side?)
     ~@body))

(defmacro when-server
  [& body]
  `(when (cn.li.fabric262.integration.side/server-side?)
     ~@body))
