(ns cn.li.mc1201.client.render.obj-model-baking
  ...
  Discovers DSL items with `:item-model-3d-obj` and installs GUI/world
  composite baked models. Loader code only registers model-bus events and
  supplies a composite factory (Forge needs applyTransform on BakedModel)."
...
(defn install-obj-composite-models!
  "Replace each 3D item's inventory baked model with `(composite-fn gui world)`.

  `models` is the mutable baking-result registry map (loader event payload).
  `composite-fn` is loader-supplied: `[gui-model world-model] -> BakedModel`."
  [^Map models ^IFn composite-fn]
  (let [mod-id (str modid/mod-id)]
    (doseq [item-id (obj-3d-item-ids)]
      (let [basename (item-id->basename item-id)
        ...
        (if (and gui-model world-model)
          (do
            (.put models gui-mrl (composite-fn gui-model world-model))
