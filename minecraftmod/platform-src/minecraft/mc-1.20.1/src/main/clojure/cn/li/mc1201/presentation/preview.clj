(ns cn.li.mc1201.presentation.preview
...
(defn- draw-perspective-preview!
  "Frame the 3D preview in the same GuiGraphics coordinate space as recipe
   ...
   Main's reactive path isolates a GL viewport + FOV-50 ModelView camera inside
   draw-tape!. Presentation paints 2D with absolute GUI verts under the screen
   ortho projection; reusing that viewport camera here left the ore sitting in
   the pane's bottom-right while 2D art stayed correct. Centering with a pose
   translate to the pane midpoint fixes the offset and keeps main's tilt/spin."
  ...
  (let [[kind id] (parse-model-id model-id)
        ...
        ;; ~0.48 of the shorter axis ≈ main showArea framing inside 134×134.
    ...
    (when stack
      ...
      (let [^PoseStack ps (.pose graphics)]
        ...
        (.setIdentity ps)
        ;; PoseStack.translate is (DDD) in 1.20.1 — floats do not resolve.
        (.translate ps (double cx) (double cy) 100.0)
        (.scale ps model-px (- model-px) model-px)
        ;; Main showArea tilt (glRotated -20 about (1,0,0.1)).
        (.mulPose ps (doto (Quaternionf.)
                       (.rotateAxis (float (Math/toRadians -20.0)) 1.0 0.0 0.1)))
        ;; drawsBlockImpl pre-spin offset (drop the -1 Z — that is eye-space only).
