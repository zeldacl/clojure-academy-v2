(ns cn.li.mc1211.client.effects.particle
  "CLIENT-ONLY shared particle effect bridge for Minecraft 1.21.1."
    ...
    (if (= :block-crack particle-type)
      (let [block-registry ^net.minecraft.core.Registry BuiltInRegistries/BLOCK
            ^BlockState state (or (when block-id
                                     (some-> ^Block (.get block-registry ^ResourceLocation (ResourceLocations/parse ^String block-id))
                                             (.defaultBlockState)))
