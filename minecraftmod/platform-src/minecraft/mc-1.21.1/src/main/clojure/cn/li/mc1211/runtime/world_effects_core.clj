...
(defn play-sound-in-level!
  ...
  (let [^SoundEvent sound-event (.get BuiltInRegistries/SOUND_EVENT (ResourceLocations/parse ^String sound-id))]
    (when sound-event
      (.playSound level
                  nil
                  (double x)
...
(defn spawn-projectile-in-level!
  ...
  (let [{:keys [entity-id x y z vx vy vz owner-uuid explosion-power]} projectile-spec]
    (try
      ...
      (let [^ResourceLocation type-key (ResourceLocations/parse (str entity-id))
  ...
  (try
    (when-let [^Level level (some-> player .level)]
      (let [item-id (:id item-stack)
            ...
            ^Item item (.get BuiltInRegistries/ITEM (ResourceLocations/parse (str item-id)))
