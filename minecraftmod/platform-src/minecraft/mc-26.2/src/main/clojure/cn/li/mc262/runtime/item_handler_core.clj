(ns cn.li.mc262.runtime.item-handler-core
  ...
  (when stack
    (try
      (let [item (.getItem stack)
            ^Identifier registry-name (.getKey BuiltInRegistries/ITEM item)]
        (when registry-name
...
(defn- game-time-ms
  "Game-time in milliseconds (pauses when server pauses). Falls back to wall-clock.
  26.2 removed Level.getGameTime; use MinecraftServer tick count via McAccess."
  [^Player player]
  (if-let [server (McAccess/serverOf player)]
    (* (McAccess/serverTickCount server) 50)
...
(defn- run-plan-actions!
  ...
  (when plan
    ...
    ;; session â€?only present on the client thread. In singleplayer the
