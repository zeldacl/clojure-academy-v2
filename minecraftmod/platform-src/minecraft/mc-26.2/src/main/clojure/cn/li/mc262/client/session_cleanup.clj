(ns cn.li.mc262.client.session-cleanup
  ...
  (:require [cn.li.mc262.client.effects.level-renderer :as level-renderer]
            [cn.li.mc262.client.overlay.state :as overlay-state]
            [cn.li.mc262.gui.reactive.overlay-host :as overlay-host]
...
(defn- close-current-screen!
  []
  ;; 26.2: screen ownership moved from Minecraft to Minecraft.gui
  (when-let [^Minecraft mc (Minecraft/getInstance)]
    (when-let [gui (.gui mc)]
      (when (some? (.screen gui))
        (.setScreen gui nil))))
  nil)
