(ns cn.li.mc262.command.brigadier-tree
...
(defn execute-command
  ...
  (try
    (let [^CommandSourceStack source (.getSource brigadier-ctx)
          ...
          ;; ???? #2): rebuild the player-state owner here ù?the Brigadier
...
(defn- suggestion-provider
  ...
  console) under a bound server player-state owner ù?the suggestion packet
...
(defn- complete-at?
  "True when supplying argument `index` already forms a runnable command ù?
...
(defn build-arguments-chain
  ...
  stops on. Attaching the executor solely to the last node ù?as this did ù?
  ...
  (let [command-id (:id command-spec)
    ...
    (.requires literal
      (reify java.util.function.Predicate
        (test [_ source]
          (McAccess/hasCommandPermission ^CommandSourceStack source (int permission-level)))))
