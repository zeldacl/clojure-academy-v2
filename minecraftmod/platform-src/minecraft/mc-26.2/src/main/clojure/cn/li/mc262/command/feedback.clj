(ns cn.li.mc262.command.feedback
  "Single shared command feedback implementation, vanilla-aligned.

  translate? true â†?send Component.translatable(key, args): the CLIENT resolves
  ...
  locale â€?matching vanilla, where the packet carries key+args only and the
  ...
  translate? false â†?literal text; args (if any) are applied with format.

  error? true â†?sendFailure (red). Vanilla API asymmetry: sendSuccess takes
