(ns cn.li.mc262.runtime.lifecycle-core
  ...
  (:require [cn.li.mc262.runtime.server-runtime :as server-runtime]
...
(defn install-server-stop-cleanup!
  "Register the on-server-stop cleanup callback exactly once per process.
   ...
   can't catch repeats here â€?this guard is load-bearing. Process-scoped
...
(defn- same-server-runtime?
  ...
  (and runtime
       (not (.disposed ^cn.li.mc262.runtime.server_runtime.IServerRuntime runtime))
       (identical? server (.server ^cn.li.mc262.runtime.server_runtime.IServerRuntime runtime))))
...
(defn- cached-player-uuid
  ...
  (let [^HashMap cache (.players ^cn.li.mc262.runtime.server_runtime.IServerRuntime runtime)
        uuid (.getUUID player)]
    (or (.get cache uuid)
        (let [value (str uuid)]
...
(defn player-tick!
  ...
  (let [uuid (cached-player-uuid runtime player)
        hooks (.hooks ^cn.li.mc262.runtime.server_runtime.IServerRuntime runtime)
        callbacks (.callbacks ^cn.li.mc262.runtime.server_runtime.IServerRuntime runtime)]
...
(defn world-tick!
  [runtime level]
  (when-let [tick-world! (get (.callbacks ^cn.li.mc262.runtime.server_runtime.IServerRuntime runtime)
                              :world-tick!)]
...
(defn server-tick-end!
  ...
  (let [hooks (.hooks ^cn.li.mc262.runtime.server_runtime.IServerRuntime runtime)
        callbacks (.callbacks ^cn.li.mc262.runtime.server_runtime.IServerRuntime runtime)]
...
(defn run-server-tick!
  ...
  (let [runtime (ensure-server-runtime! server callbacks)
    ...
    (.beginTick ^cn.li.mc262.runtime.server_runtime.IServerRuntime runtime game-time)
    (let [owner (server-runtime/runtime-owner runtime)
          old-context (player-hooks/push-player-state-owner! owner)]
      (try
        ((get (.hooks ^cn.li.mc262.runtime.server_runtime.IServerRuntime runtime)
        ...
        (finally
          (.finishTick ^cn.li.mc262.runtime.server_runtime.IServerRuntime runtime)
