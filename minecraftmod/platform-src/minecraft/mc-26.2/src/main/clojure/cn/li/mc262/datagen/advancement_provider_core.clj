(ns cn.li.mc262.datagen.advancement-provider-core
  "Shared advancement datagen JSON assembly helpers (26.2)."
  (:require [clojure.string :as str]
            [cn.li.mcmod.config :as modid]
            [cn.li.mc262.datagen.resource-location :as rl]
            [cn.li.mc262.datagen.item-registry :as item-registry]))
...
(defn ach-path
  [id]
  (str "data/" modid/mod-id "/advancement/achievements/" (normalize-id id) ".json"))

(defn root-path
  [tab-id]
  (str "data/" modid/mod-id "/advancement/achievements/" (name tab-id) "/root.json"))
...
(defn root-json
  [{:keys [id background]}]
  {"display" {"icon" {"id" "minecraft:nether_star"}
...
(defn ach-json
  ...
  (let [id (:id ach)
        ...
        parent-rl (if-let [parent (:parent ach)]
                    ...
                    root-rl)]
    {"parent" parent-rl
     "display" {"icon" {"id" (item-registry/safe-item-id (:icon ach) known rl/parse-resource-location)}
