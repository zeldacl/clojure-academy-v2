(ns cn.li.mc262.runtime.potion-effects-core
...
(defn- effect-holder
  "Resolve a Holder<MobEffect> for the given registry Identifier, or nil."
  ^Holder [^Identifier rl]
  (let [^Optional opt (.get BuiltInRegistries/MOB_EFFECT rl)]
    (.orElse opt nil)))
...
(defn- get-mob-effect
  ...
  (let [vanilla-key (get vanilla-effect-keys effect-type)]
    (if vanilla-key
      (effect-holder (ResourceLocations/parse vanilla-key))
