 ...
 *
 * <p>26.2 wiring: FontSet takes a {@link GlyphStitcher} and exposes
 * {@link FontSet#source(boolean)} as the {@link GlyphSource}; the Font is
 * built from a {@link Font.Provider} that serves that source for any
 * {@link FontDescription}.</p>
 */
public final class MsdfFontManager {

    private static final Logger LOGGER = LogManager.getLogger();

    public static final Identifier SHADOW_FONT_ID =
    ...
    private static GlyphStitcher stitcher;
    ...
    public static boolean init(final Path fontPath) {
        synchronized (MsdfFontManager.class) {
            ...
            try {
                ...
                stitcher = new GlyphStitcher(mc.getTextureManager(), SHADOW_FONT_ID);
                final FontSet fontSet = new FontSet(stitcher);
                fontSet.reload(
                ...
                final GlyphSource source = fontSet.source(false);
                shadowFont = new Font(new Font.Provider() {
                    @Override
