...
(defn create-tile-inventory-adapter
  ...
  (reify Container
    (getContainerSize [_]
      (int (or (platform/slot-count clj-container) 0)))

    (isEmpty [_]
      (let [n (int (or (platform/slot-count clj-container) 0))]
        (not-any? (fn [idx]
                    (let [stack (platform/slot-get-item clj-container idx)]
                      (and stack (not (.isEmpty ^ItemStack stack)))))
                  (range n))))

    (getItem [_ slot]
      (or (platform/slot-get-item clj-container (int slot))
          ItemStack/EMPTY))

    (removeItem [_ slot amount]
      (let [slot   (int slot)
            amount (int amount)
            current (or (platform/slot-get-item clj-container slot)
                        ItemStack/EMPTY)]
        (if (or (nil? current) (.isEmpty ^ItemStack current) (<= amount 0))
          ItemStack/EMPTY
          (let [taken (.split ^ItemStack current amount)]
            (if (.isEmpty ^ItemStack current)
              (platform/slot-set-item! clj-container slot nil)
              (platform/slot-set-item! clj-container slot current))
            (platform/slot-changed! clj-container slot)
            taken))))
    ...
    (setItem [_ slot stack]
      (platform/slot-set-item! clj-container (int slot) stack)
      (platform/slot-changed! clj-container (int slot)))

    (setChanged [_]
      (let [n (int (or (platform/slot-count clj-container) 0))]
        (doseq [idx (range n)]
          (platform/slot-changed! clj-container idx))))

    (stillValid [_ player]
      (boolean (platform/safe-validate clj-container player)))

    (canPlaceItem [_ slot stack]
      (boolean (platform/slot-can-place? clj-container (int slot) stack)))

    (clearContent [_]
      (let [n (int (or (platform/slot-count clj-container) 0))]
        (doseq [idx (range n)]
          (platform/slot-set-item! clj-container idx nil))
        (doseq [idx (range n)]
          (platform/slot-changed! clj-container idx))))))
