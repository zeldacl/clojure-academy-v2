...
public final class MsdfEngine {
    ...
    public static MsdfPixels generate(
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            for (int i = 0; i < w * h; i++) {
                // STB convention: inside<128, edge=128, outside>128.
                // Shader convention: inside>128 (opaque), outside<128 (transparent).
                // Flip so smoothstep(fontBase-w, fontBase+w, sd) resolves correctly.
                final byte v = (byte) (255 - (sdf.get(i) & 0xFF));
                final int idx = i * 4;
                rgba[idx] = v;
                rgba[idx + 1] = v;
                rgba[idx + 2] = v;
