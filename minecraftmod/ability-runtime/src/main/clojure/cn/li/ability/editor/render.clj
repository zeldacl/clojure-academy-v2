...
(defn graph->composite-items
  "A surface graph (cn.li.ability.editor.graph/form->graph's output) ->
   canvas composite items.

   Used to branch on whether the graph carried an :order, rendering a
   persisted V4 node/link graph when it did not. form->graph always
   produces an :order, and persisted node/link graphs no longer exist, so
   that arm was 149 lines that could not run."
  [graph stored-layout]
