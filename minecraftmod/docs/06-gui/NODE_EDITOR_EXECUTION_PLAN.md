...
3. 节点检查器基础版已按类型提供 bool 切换、数值 stepper，文本框保留为 keyword/vec3/无 schema 的安全回退；keyword 选择和 vec3 三轴控件列为增强项。
...
P0/P1 已完成并通过门禁；P2 基础版（搜索/分类/最近使用、wheel 缩放、bool/数值控件、撤销重做）已完成并有回归测试，剩余是 pinch、keyword 选择、vec3 三轴和真实游戏人机评估。P3 的物品/NBT、准星和 emitter 仍依赖内容/平台契约。
