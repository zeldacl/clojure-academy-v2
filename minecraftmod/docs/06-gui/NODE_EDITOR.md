...
- **调色板交互（基础版已完成）**：调色板支持搜索、分类折叠、最近使用、点击或鼠标拖拽插入执行节点，并自动创建
  声明参数的字面量数据节点；拖拽期间显示 ghost，释放位置有有效/无效反馈，Esc 可取消，
  无移动的点击仍走快捷插入。执行链节点移动、选择、空白画布平移，以及表达式输出到语句
  输入的语义连线也已实现。若 schema 提供 `:choices`，keyword 使用循环选择；vec3 字面量显示 x/y/z 三轴输入；无元数据时仍保留文本回退。后续体验迭代是 pinch 缩放和真实游戏人机评估。
...
- **画布缩放**：画布平移和基于既有 `:scroll` 事件的 50%–200% 光标锚定缩放已实现，
  保存的 layout 仍保持图坐标；运行时已支持 Tab/Shift-Tab 在可见控件间环回，当前剩余是等待独立中立 pinch 手势契约。
