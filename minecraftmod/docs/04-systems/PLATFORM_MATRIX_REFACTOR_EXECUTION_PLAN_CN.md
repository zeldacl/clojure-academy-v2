...
修复后的最新回归证据为：`cmd /c gradlew.bat test verifyCorePerformance --stacktrace` 通过（79 tasks），`cmd /c gradlew.bat verifyCurrentPlatforms --stacktrace` 通过（99 tasks）；两者均覆盖当前构建层和共享运行时改动。
