...
Loader-neutral compile/install pipeline in **`platform-src/minecraft/version/mc-1201`**; Forge/Fabric only register types and call the pipeline. **No dual-path**: runtime must not fall back to `RT.var` or `^:dynamic` tile-logic registries.
...
| `ITileCapabilityLogic` | `resolve(be, capKey, side) 鈫?Object` | Forge wraps in cached `LazyOptional` |
...
| `IMobHurtLogic` | `onIncomingDamage` 鈫?`float` | `NaN` = cancel hurt |
| `IMobDeathLogic` | `onDie` | called before `super.die()` |
| `IMobLootLogic` | `dropLoot` 鈫?`boolean` | `true` = skip vanilla `dropFromLootTable` |

`ScriptedEntityLogicRegistry` (`IdentityHashMap<EntityType<?>, MobLogicBundle>`) is the sole mob bundle anchor.

## Registration phases (P1鈫扨5)

```mermaid
flowchart TD
  P1["P1 content init\ntile-dsl / entity-dsl / tile-kind"]
  P2["P2 compile-all-bundles\nplatform-src/minecraft/version/mc-1201 loader-neutral"]
...
- `{tile-id 鈫?bundle}` lives in a **local `let`** during registration 鈥?not `defonce`, not `^:dynamic`.
- BE hot path: `getBlockState().getBlock()` 鈫?`IScriptedBlock.getTileLogic()` 鈫?interface call (field read + `instanceof` + invokevirtual).

## BlockEntity path

1. **Declaration** (`mcmod`): `tile-dsl` / `tile-kind` 鈥?`:tick-fn`, `:read-nbt-fn`, `:write-nbt-fn`, `:container`, `:capability-keys`.
...
1. **Declaration**: entity DSL `:scripted-mob` + `:properties.mob` (`:mob-tick-fn`, 鈥?.
