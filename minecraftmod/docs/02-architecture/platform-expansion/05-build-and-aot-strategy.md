...
`forge-1.20.1/build.gradle` 当前承担了大量平台构建特殊逻辑，包括但不限于：
...
1. **构建规则复制成本高**：新增 `NeoForge` / 新版本时容易复制大量 build logic。
...
新增 `forge-*` / `neoforge-*` / `fabric-*` 模块时，必须回答以下问题：
