...
- 本轮修正：生成 MdBall 后通过 Final `target/entities` 查询取得本次生成球的实体 ID，并把 ID 放入中立 `origin-selector`；由 `ability-runtime` continuation queue 承载延迟结算，并在 Combat settlement 中用 `:entity-id` 精确过滤。同一玩家同时存在多个 MdBall 时，延迟射线不会误选其它技能的球，也不会跨玩家选中实体。
...
- 已删除重复 `:cost/spend` 节点，保留单一事务扣费、AOE damage/knockback、方块破坏、
  hit/miss progression、cooldown 和统一 VFX 链；本轮进一步补齐 hit/miss 的
  `ability/progression` 提交，避免只带 tag 的 `score/mark` 被 runtime 解释为 0 经验。
- `:ac:runAcEdnCoverageTests` 通过（26 tests / 71 assertions）。资源适配器实际扣费、
...
- 本轮发现并修正命中经验遗漏：main 的 `add-skill-exp!` 现在由 release 与 auto-release
  两条 Final 分支通过 `ability/progression :hit` + `score/mark` 提交，避免仅有 tag 的
  score 事件被 runtime 解释为 0 经验。
- `:ac:runAcEdnCoverageTests` 通过（25 tests / 67 assertions）。charge-slow 在各版本
...
- `:ac:runAcEdnCoverageTests` 通过（41 tests / 128 assertions）。飞行碰撞、爆炸地形权限和
...
- main 在进入飞行阶段后每个持续 tick 调用一次 add-skill-exp exp-tick；充能阶段、资源失败和 abort 不应产生 flight-tick 经验。Final 的 pulse 在 phase=1 时保留同样的每 tick 语义。
- 本轮把原先只有 score/mark flight-tick 的节点改为 ability/progression flight-tick 加 score progression ref，继续沿用现有 can-fly、软方块、移动、击退、资源地板和 VFX 清理图。
- 已通过 ac runAcEdnCoverageTests（34 tests / 100 assertions）。飞行移动、资源耗尽时序、多人 VFX audience 仍需实机验证，总表保持 ⚠️。
...
- `groundshock` 的 Final 图已覆盖 main 的地面传播、方块替换/破坏、AOE 伤害、随机
  ...
  共享可变集合。本轮补齐 `exp-entity` 与 `exp-use` 两条 progression：实体命中在 foreach
...
- `:ac:runAcEdnCoverageTests` 通过（41 tests / 128 assertions）；Combat Core damage/reduce 与 damage/reflect 的受击经验桥已由 35 tests / 92 assertions 覆盖。Groundshock 的实体/方块
...
- Final pulse 现在只负责以 `owner-id + world-id + entity-type` 查询持有实体、更新
  homing/session/VFX，不再扣资源；release 在读取实体距离后通过唯一的
  `cost/spend :release` 扣一次 CP/过载，且只对有效距离收费，匹配 main 的 release
  callback 时序。
- release 费用不足时由同一 Final 分支设置 `place-when-collide?`、销毁 hold VFX 并结束
  session；pulse 不会提前消耗资源或留下第二条执行路径。
...
- Final 图覆盖蓄力 start/pulse/release/abort、最大蓄力自动释放、最小蓄力判断、
  ...
  VFX、成就和 session 清理。
- 已修复 AOE 过滤遗漏施法者的问题：Final 两个 `combat/charged-area-damage` 分支均
  通过 `excluded-entity-ids=[caster/id]` 传入中性 area composite，匹配 main 的
  `attackRange` 自体排除。
...
- 已通过 `:ac:runAcEdnCoverageTests`（22 tests / 55 assertions）。该技能的主线 cooldown
