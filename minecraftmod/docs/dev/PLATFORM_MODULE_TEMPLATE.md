...
本文件给出平台模块的标准骨架，供 `Forge` / `future loader` / `Fabric` 新模块复用。
...
│   └── platform/spi/
│       └── <platform target bootstrapImpl>.java
├── src/main/clojure/cn/li/<loaderVersion>/
│   ├── mod.clj
│   ├── init.clj
│   ├── platform/
│   │   ├── platform/init.clj
│   │   └── platform/init.clj       # 若采用门面 + 真实安装分层
│   ├── registry.clj
│   ├── events.clj
│   ├── side.clj                     # Forge / future loader 推荐
│   ├── config/bridge.clj
│   ├── gui/
│   │   ├── init.clj
│   │   ├── network.clj
...
| `platform/spi/<platform target bootstrapImpl>.java` | 若沿用 SPI 则必须 | ServiceLoader 发现与平台安装入口 | Java SPI、Clojure runtime | 直接承载平台业务初始化 |
| `mod.clj` | 必须 | 平台主初始化排序与总装配 | 平台桥接、共享层入口 | 在 namespace load 时做过重副作用 |
| `init.clj` | 推荐 | Java/Clojure 之间的轻量桥接 | 共享初始化函数 | 与 `mod.clj` 职责混杂 |
| `platform/platform/init.clj` | 必须 | 平台初始化门面 | SPI、日志 | 直接写所有平台对象扩展 |
| `platform/platform/init.clj` | 推荐/常用 | 安装平台对象协议与工厂函数 | Minecraft/Loader API | 暴露为共享层 API |
| `registry.clj` | 必须 | metadata → 平台 registry API | 平台注册 API、共享元数据 | 写业务定义 |
| `events.clj` | 必须 | 平台事件接线 | Loader 事件 API | 在共享层复制事件分发 |
| `side.clj` | Forge/future loader 推荐 | 物理侧识别与 client-only 解析 | Loader side API | 承载具体渲染业务 |
...
1. `build.gradle`
2. `<MainEntry>.java`
3. `platform/spi/<platform target bootstrapImpl>.java`
4. `platform/platform/init.clj`
5. `platform/platform/init.clj`
6. `mod.clj`
7. `registry.clj`
8. `events.clj`
9. `gui/*`
...
- `src/main/java/cn/li/future-loader1201/MyModfuture loader.java`
- `src/main/java/cn/li/future-loader1201/platform/spi/future loader1201platform target bootstrap.java`
- `src/main/clojure/cn/li/future-loader1201/mod.clj`
- `src/main/clojure/cn/li/future-loader1201/platform/platform/init.clj`
- `src/main/clojure/cn/li/future-loader1201/platform/platform/init.clj`
...
- `platform-bootstrap/init-platform!` 能通过 SPI 找到 `future loader1201platform target bootstrap`
...
- 新增 `future loader` 时，以当前 `forge target` 为一号模板。
- 整理 `Fabric` 时，以现有 `fabric target` 为素材，但统一到本模板命名规范。
- 任何偏离模板的结构都应有文档解释原因。
