...
| 目录 | 职责 |
|------|------|
| `platform-src/common/` | 与 Loader 无关的平台通用代码。 |
| `platform-src/minecraft/base/` | Minecraft API 通用适配。 |
| `platform-src/minecraft/version/mc-1201/` | Minecraft 1.20.1 专属差异。 |
...
`platform-targets.json` 是唯一目标目录。每个 target 显式声明 loader、Minecraft version、Java version、source components、test components、capabilities、dependencies、artifact 信息与 datagen parity group。
