...
本文档合并自：原始实现分析、移植实现报告、迁移总结。当前项目资源路径以 `assets/my_mod/guis/`、`my_mod:guis/rework/` 为准；能量相关逻辑见 `my-mod.energy.operations`。
...
- 能量相关：统一使用 `my-mod.energy.operations`（物品支持、充放电等）。
