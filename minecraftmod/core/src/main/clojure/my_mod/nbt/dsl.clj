...
(defmacro defworldnbt
  ...
  (let [opts (apply hash-map options)
        ...
        ;; Generate write statements directly using list construction
        write-stmts (for [spec lists]
                      (let [{:keys [tag atom to-nbt skip?]} spec]
                        `(write-nbt-list-field ~'nbt ~tag @(~atom ~'world-data) ~to-nbt ~skip?)))
        
        ;; Generate read statements directly using list construction
        read-stmts (for [spec lists]
