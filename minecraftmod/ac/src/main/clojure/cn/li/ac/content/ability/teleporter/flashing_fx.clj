...
(defn flashing-fx-snapshot []
  (or (level-effects/effect-state-snapshot flashing-effect-id)
      (default-flashing-fx-runtime-state)))
...
(defn clear-flashing-owner! [owner-key]
  (level-effects/update-effect-state!
    flashing-effect-id
    (fn [state]
      (update (or state (default-flashing-fx-runtime-state)) :fx-state dissoc owner-key)))
  nil)

(defn- enqueue-state!
  [state {:keys [payload ctx-id channel owner-key]}]
  (let [state* (or state (default-flashing-fx-runtime-state))
    ...
    (case mode
      :state-start
      (update state* :fx-state assoc owner-key* (merge base-meta {:preview nil :burst []}))
      ...
      (update state* :fx-state update owner-key*
              (fn [st]
                (assoc (merge base-meta (or st {:burst []})) :preview nil)))
