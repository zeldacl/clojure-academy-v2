...
(defn- read-resource [r] (let [u (or (classpath-edn/find-resource r) (io/resource r))] (when-not u (throw (ex-info "V4 VFX resource not found" {:resource r}))) (binding [*read-eval* false] (read-string (slurp u)))))
(defn- read-effect [r] (let [d (read-resource r)] (node-api/validate-v4-document! d) (when-not (= :vfx (node-api/v4-document-kind d)) (throw (ex-info "V4 VFX catalog contains non-VFX document" {:resource r :schema (:schema d)}))) {:resource r :id (:id d) :document d :user-types (into {} (map (fn [[k v]] [k (:type v)]) (or (:inputs d) (:parameters d)))) :graphs (:graphs d) :lifecycle (get-in d [:lifecycle :mode])}))
