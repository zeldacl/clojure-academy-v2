...
(defn- cfg-lerp-int
  [field-id exp]
  (helper/cfg-lerp-int flashing-skill-id field-id exp))
...
(defn- resolve-preview
  ...
  (let [exp (skill-exp player-id)
        dist (helper/cfg-lerp flashing-skill-id :movement.blink-distance exp)
...
(defn- perform-flash!
  ...
  (let [preview (resolve-preview player-id direction)
        ...
        overload-cost (helper/cfg-lerp flashing-skill-id :cost.blink.overload exp)]
    (when (and preview
