...
(defn shift-teleport-fx-snapshot []
  (or (level-effects/effect-state-snapshot shift-teleport-effect-id)
      (default-shift-teleport-fx-runtime-state)))

(defn reset-shift-teleport-fx-for-test! []
  (level-effects/reset-level-effect-state-for-test!
    shift-teleport-effect-id
    (default-shift-teleport-fx-runtime-state))
  nil)
...
(defn clear-shift-teleport-owner! [owner-key]
  (level-effects/update-effect-state!
    shift-teleport-effect-id
    (fn [state]
      (update (or state (default-shift-teleport-fx-runtime-state)) :fx-state dissoc owner-key)))
  nil)
...
(defn- enqueue-state! [state {:keys [payload ctx-id channel owner-key]}]
  (let [state* (or state (default-shift-teleport-fx-runtime-state))
    ...
    (case (:mode payload)
      :start
      (update state* :fx-state assoc owner-key* (merge base-meta {:active? true :ttl 0 :target nil :target-count 0 :hand-valid? true}))
      :update
