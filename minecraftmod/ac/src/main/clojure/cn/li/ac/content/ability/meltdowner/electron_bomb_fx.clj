...
(defn- clear-electron-bomb-active-owner! [owner-key]
  (update-electron-bomb-fx-state! update :effect-state dissoc owner-key)
  nil)

(defn clear-electron-bomb-owner! [owner-key]
  (clear-electron-bomb-active-owner! owner-key)
  (update-electron-bomb-fx-state! update :beams dissoc owner-key)
  nil)
...
(defn- tick! []
  ...
  (update-electron-bomb-fx-state!
    update :beams
    (fn [by-owner]
      (reduce-kv (fn [acc owner-key beams]
                   (let [live (->> beams
                                   (map #(update % :ttl dec))
                                   (filter #(pos? (long (:ttl %))))
                                   vec)]
...
(defn- build-plan [camera-pos _hand-center-pos _tick]
  (let [{:keys [effect-state beams]} (electron-bomb-fx-state-snapshot)
