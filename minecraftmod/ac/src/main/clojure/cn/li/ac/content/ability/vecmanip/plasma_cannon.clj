(ns cn.li.ac.content.ability.vecmanip.plasma-cannon
  ...
  Mechanics:
  - Charge 60é”?0 ticks (scales with exp, held-key)
  - Per-tick CP drain during charge: 18é”?5 (scales UP with exp)
  - Initial overload: 500é”?00 (maintained throughout skill, prevents recovery)
  - On key-release (fully charged): raycast from player eye to first living entity
    or block within 100 blocks é”?becomes destination
  - Plasma projectile spawns 15 blocks above player, flies at 1 block/tick
  - Max flight time: 240 ticks
  - Explosion: radius 12é”?5, damage 80é”?50 in 10-block radius (no friendly fire)
  - Cooldown: 1000é”?00 ticks (set on fire, scales down with exp)
...
(defn plasma-cannon-on-key-down
  "Server-side: initialize charge. Consume initial overload (500é”?00 by exp).
    ...
    (when-let [ctx-data (ctx-skill/get-context ctx-id)]
      (let [skill-state (:skill-state ctx-data)
        ...
        (case state
          ...
          ;; Fallback: unknown state é”?terminate
          (do
            (fx/send! ctx-id {:topic :plasma-cannon/fx-end :mode :end} nil {:performed? false})
            (ctx/terminate-context! ctx-id nil)))))
    (catch Exception e
    ...
    (when-let [ctx-data (ctx-skill/get-context ctx-id)]
      (let [skill-state  (:skill-state ctx-data)
        ...
        (if (or (= state :go)
          ...
          ;; Not fully charged or already flying é”?abort
          (when-not (= state :go)
            (fx/send! ctx-id {:topic :plasma-cannon/fx-end :mode :end} nil {:performed? false})
            (ctx/terminate-context! ctx-id nil)
            (log/debug "PlasmaCannon: Released before fully charged, aborting"))

          ;; Fully charged é”?fire (equivalent to s_perform on server)
          (let [pos      (get-player-position player-id)
            ...
            (fx/send! ctx-id {:topic :plasma-cannon/fx-update :mode :update} nil {:state       :go
                              :charge-pos  spawn-pos
                              :destination dest})
            (log/info "PlasmaCannon: Fired é”?destination"
