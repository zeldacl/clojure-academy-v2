...
(deftype WirelessNodeImpl [be]
  ...
  (getEnergy [_]
    (let [state (or (platform-be/get-custom-state be) node-block/node-default-state)]
      (double (schema/get-field node-block/node-state-schema state :energy))))

  (getMaxEnergy [_]
    (let [state (or (platform-be/get-custom-state be) node-block/node-default-state)]
      (double (node-block/node-max-energy state))))

  (getBandwidth [_]
    (let [state     (or (platform-be/get-custom-state be) node-block/node-default-state)
          node-type (keyword (schema/get-field node-block/node-state-schema state :node-type))]
      (double (get-in node-block/node-types [node-type :bandwidth] 150))))

  (getCapacity [_]
    (let [state     (or (platform-be/get-custom-state be) node-block/node-default-state)
          node-type (keyword (schema/get-field node-block/node-state-schema state :node-type))]
      (int (get-in node-block/node-types [node-type :capacity] 5))))

  (getRange [_]
    (let [state     (or (platform-be/get-custom-state be) node-block/node-default-state)
          node-type (keyword (schema/get-field node-block/node-state-schema state :node-type))]
      (double (get-in node-block/node-types [node-type :range] 9))))

  (getNodeName [_]
    (let [state (or (platform-be/get-custom-state be) node-block/node-default-state)]
      (str (schema/get-field node-block/node-state-schema state :node-name))))

  (getPassword [_]
    (let [state (or (platform-be/get-custom-state be) node-block/node-default-state)]
      (str (schema/get-field node-block/node-state-schema state :password))))
...
(deftype ClojureEnergyImpl [be]
  ...
  (receiveEnergy [_ max-receive simulate]
    (let [state    (or (platform-be/get-custom-state be) node-block/node-default-state)
          cur      (double (schema/get-field node-block/node-state-schema state :energy))
          max-e    (double (node-block/node-max-energy state))
          can-recv (- max-e cur)
  ...
  (getEnergyStored [_]
    (let [state (or (platform-be/get-custom-state be) node-block/node-default-state)]
      (int (schema/get-field node-block/node-state-schema state :energy))))

  (getMaxEnergyStored [_]
    (let [state (or (platform-be/get-custom-state be) node-block/node-default-state)]
      (int (node-block/node-max-energy state))))
