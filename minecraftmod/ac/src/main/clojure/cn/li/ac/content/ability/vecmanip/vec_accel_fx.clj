...
(defn vec-accel-fx-snapshot
	[]
	(or (level-effects/effect-state-snapshot vec-accel-effect-id)
			(default-vec-accel-fx-runtime-state)))
...
(defn clear-vec-accel-owner!
	...
	(level-effects/update-effect-state!
		vec-accel-effect-id
		(fn [store]
			(update (or store (default-vec-accel-fx-runtime-state)) :effect-state dissoc owner-key)))
	nil)

(defn- enqueue-state!
	[store {:keys [payload ctx-id channel owner-key]}]
	(let [store* (or store (default-vec-accel-fx-runtime-state))
...
(defn- tick-state!
	[store]
	(or store (default-vec-accel-fx-runtime-state)))
