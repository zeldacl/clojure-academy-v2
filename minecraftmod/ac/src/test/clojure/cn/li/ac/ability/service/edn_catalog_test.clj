...
(deftest first-phase-catalog-is-authoritative
  (let [state (catalog/initialize!)]
    ...
    (is (catalog/available? :space-fluct))
    ...
    (is (= :migrated (catalog/migration-status :space-fluct)))
    (is (= :migrated (catalog/migration-status :flesh-ripping)))
    ...
    (is (pos? (count (get-in state [:combat :abilities :space-fluct :compiled-ir]))))
    (is (= :flesh-ripping (get-in state [:combat :abilities :flesh-ripping :id])))
    (is (pos? (count (get-in state [:combat :abilities :flesh-ripping :compiled-ir]))))
