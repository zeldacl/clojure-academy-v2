...
(deftest presentation-surface-coverage-catalog-test
  (testing "the executable inventory matches the quantitative acceptance table"
    (is (= 15 (count surface-manifest/application-surfaces)))
    (is (= 11 (count surface-manifest/container-surfaces)))
    (is (= 26 (count surface-manifest/active-surfaces)))
    (is (= 17 (count (set (map :artifact surface-manifest/active-surfaces)))))
    (is (= 26 (count (filter :controller surface-manifest/active-surfaces))))))
