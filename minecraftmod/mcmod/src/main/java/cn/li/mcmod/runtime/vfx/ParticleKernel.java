...
public final class ParticleKernel {
    ...
    /** Integrate the active buffer and compact expired particles in-place. */
    public static int integrate(ParticleBuffer particles, int start, int end, float deltaSeconds) {
        ...
        if (start != 0 || end != particles.size()) {
            throw new IllegalArgumentException("ParticleKernel.integrate requires the full active range");
        }
        int limit = particles.size();
        int index = 0;
