...
(declare validate-nbt-field-spec validate-world-list-spec)
...
(def type-converters
  ...
  {:double
   {:write (fn [nbt key value]
             (nbt/set-double! nbt key value))
    :read (fn [nbt key]
            (nbt/get-double nbt key))
    :has-key? (fn [nbt key]
                (nbt/has-key-safe? nbt key))}

   :string
   {:write (fn [nbt key value]
             (nbt/set-string! nbt key value))
    :read (fn [nbt key]
            (nbt/get-string nbt key))
    :has-key? (fn [nbt key]
                (nbt/has-key-safe? nbt key))}

   :int
   {:write (fn [nbt key value]
             (nbt/set-int! nbt key value))
    :read (fn [nbt key]
            (nbt/get-int nbt key))
    :has-key? (fn [nbt key]
                (nbt/has-key-safe? nbt key))}
   ...
   :boolean
   {:write (fn [nbt key value]
             (nbt/set-boolean! nbt key value))
    :read (fn [nbt key]
            (nbt/get-boolean nbt key))
    :has-key? (fn [nbt key]
                (nbt/has-key-safe? nbt key))}

   :float
   {:write (fn [nbt key value]
             (nbt/set-float! nbt key value))
    :read (fn [nbt key]
            (nbt/get-float nbt key))
    :has-key? (fn [nbt key]
                (nbt/has-key-safe? nbt key))}

   :long
   {:write (fn [nbt key value]
             (nbt/set-long! nbt key value))
    :read (fn [nbt key]
            (nbt/get-long nbt key))
    :has-key? (fn [nbt key]
                (nbt/has-key-safe? nbt key))}

     :keyword
     {:write (fn [nbt key value]
         (nbt/set-string! nbt key (name value)))
      :read (fn [nbt key]
        (keyword (nbt/get-string nbt key)))
      :has-key? (fn [nbt key]
      (nbt/has-key-safe? nbt key))}
...
(defn write-nbt-list-field
  ...
  (let [list (nbt/create-list)]
    (doseq [item items]
      (when-not (and skip? (skip? item))
        (nbt/append! list (to-nbt-fn item))))
    (nbt/set-tag! nbt tag list))
  nbt)
...
(defn read-nbt-list-field!
  ...
  (when (nbt/has-key-safe? nbt tag)
    (let [list (nbt/get-list nbt tag)
          size (nbt/list-size list)]
      (dotimes [i size]
        (let [item-nbt (nbt/list-compound list i)
...
(defmacro defworldnbt
  ...
  (let [opts (apply hash-map options)
    ...
    (when (vector? lists-expr)
      (doseq [list-spec lists-expr]
        (validate-world-list-spec list-spec)))
...
(defn write-atom-int
  ...
  [_tile nbt nbt-key atom-value]
  (nbt/set-int! nbt nbt-key @atom-value))

(defn read-atom-int
  "Read an integer into an atom"
  [tile nbt nbt-key]
  (when (nbt/has-key-safe? nbt nbt-key)
    (reset! (get-in tile [nbt-key]) (nbt/get-int nbt nbt-key))))

(defn write-atom-double
  "Write an atom containing a double"
  [_tile nbt nbt-key atom-value]
  (nbt/set-double! nbt nbt-key @atom-value))

(defn read-atom-double
  "Read a double into an atom"
  [tile nbt nbt-key]
  (when (nbt/has-key-safe? nbt nbt-key)
    (reset! (get-in tile [nbt-key]) (nbt/get-double nbt nbt-key))))

(defn write-atom-boolean
  "Write an atom containing a boolean"
  [_tile nbt nbt-key atom-value]
  (nbt/set-boolean! nbt nbt-key @atom-value))

(defn read-atom-boolean
  "Read a boolean into an atom"
  [tile nbt nbt-key]
  (when (nbt/has-key-safe? nbt nbt-key)
    (reset! (get-in tile [nbt-key]) (nbt/get-boolean nbt nbt-key))))
...
(def ^:private nbt-field-spec-schema
  [:map
   ...
   [:getter {:optional true} fn?]
   [:setter {:optional true} fn?]
   [:custom-write {:optional true} fn?]
   [:custom-read {:optional true} fn?]
   [:skip-on-write? {:optional true} fn?]
...
(def ^:private world-list-spec-schema
  [:map
   ...
   [:to-nbt fn?]
   [:from-nbt fn?]
   [:skip? {:optional true} fn?]
   [:rebuild {:optional true}
    [:map
     [:lookup-atom keyword?]
     [:direct-keys {:optional true} [:sequential keyword?]]
