...
(defn available? [] (boolean (get-in @fw/*framework* [:platform :entity-damage])))
(defn current [] (get-in @fw/*framework* [:platform :entity-damage]))
(defn call-with-runtime [rt f] (f rt))

(defn apply-direct-damage!* [world-id entity-uuid damage source-type]
  (when-let [rt (get-in @fw/*framework* [:platform :entity-damage])]
