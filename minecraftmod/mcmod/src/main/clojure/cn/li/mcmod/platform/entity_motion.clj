...
(defn available? [] (boolean (get-in @fw/*framework* [:platform :entity-motion])))
(defn current [] (get-in @fw/*framework* [:platform :entity-motion]))
(defn call-with-runtime [rt f] (f rt))

(defn set-velocity!* [world-id entity-uuid x y z]
  (when-let [rt (get-in @fw/*framework* [:platform :entity-motion])]
