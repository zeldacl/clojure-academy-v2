(ns cn.li.mcmod.gui.network
  ...
  (:require [cn.li.mcmod.util.log :as log]
            [cn.li.mcmod.gui.container :as gcontainer]))
...
(defn default-button-click-handler [data player]
  ...
  (let [{:keys [container-id button-id]} data]
    (when-let [container (gcontainer/get-container container-id)]
      (when (gcontainer/validate-container container player)
        (gcontainer/handle-button-click! container button-id)))))
...
(defn default-slot-change-handler [data player]
  ...
  (let [{:keys [container-id slot-index item-stack]} data]
    (when-let [container (gcontainer/get-container container-id)]
      (when (gcontainer/validate-container container player)
        (gcontainer/set-slot-item! container slot-index item-stack)))))
