(ns my-mod.forge1165.gui.impl
  "Forge 1.16.5 GUI/Menu implementation"
...
(defmethod gui-api/open-gui :forge-1.16.5
  [player gui-id world pos]
  (log/info "Opening GUI" gui-id "for player at" pos "(1.16.5)")
  ;; In Forge 1.16.5, we need to use NetworkHooks.openGui
  ;; This will be called from Java layer
  {:gui-id gui-id 
   :pos pos
   :player player
   :world world
...
(defmethod gui-api/register-menu-type :forge-1.16.5
  [menu-id factory]
  (log/info "Registering menu type" menu-id "(1.16.5)")
  ;; Actual registration via IForgeMenuType in Java
  {:menu-id menu-id})
...
(defn create-menu-title
  "Create menu title for 1.16.5 (StringTextComponent)"
  []
