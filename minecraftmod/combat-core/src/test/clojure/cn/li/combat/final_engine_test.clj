...
(deftest compiler-rejects-query-after-mutation-test
  (is (try (compiler/compile-program {:component :flow/sequence :steps [{:component :combat/damage :args {:amount 1}} {:component :target/raycast :bind :hit}]}) false (catch clojure.lang.ExceptionInfo error (= :query-after-mutation (:reason (ex-data error)))))))
