(ns cn.li.fabric1201.commands
  ...
  and registers them with Minecraft's command system. Uses the same Brigadier
  API as Forge; registration happens through Fabric's ServerLifecycleEvents."
...
(defn build-command-node
  ...
  (let [command-id (:id command-spec)
    ...
    (if-let [subcommands (:subcommands command-spec)]
      ...
      ;; Handle flat command (with only arguments, no subcommands)
      (let [args (:arguments command-spec)]
        (if (seq args)
          (let [^RequiredArgumentBuilder arg-chain (build-arguments-chain args (:executor-fn command-spec) nil)]
            (.then literal arg-chain))
          (when-let [executor-fn (:executor-fn command-spec)]
