...
(defn forge-block-manipulation []
  (reify bm/IBlockManipulation
    (break-block! [_ player-id world-id x y z drop?]
      (bmc/break-block! (server-context/get-server) player-id world-id x y z drop? forge-break-guard))
    (set-block! [_ world-id x y z block-id]
      (bmc/set-block! (server-context/get-server) world-id x y z block-id))
    (get-block [_ world-id x y z]
      (bmc/get-block (server-context/get-server) world-id x y z))
    (get-block-hardness [_ world-id x y z]
      (bmc/get-block-hardness (server-context/get-server) world-id x y z))
    (can-break-block? [_ player-id world-id x y z]
      (boolean (bmc/can-break-block? (server-context/get-server) player-id world-id x y z forge-break-guard)))
    (find-blocks-in-line [_ world-id x1 y1 z1 dx dy dz max-distance]
      (bmc/find-blocks-in-line (server-context/get-server) world-id x1 y1 z1 dx dy dz max-distance))
    (liquid-block? [_ world-id x y z]
      (boolean (bmc/liquid-block? (server-context/get-server) world-id x y z)))
    (farmland-block? [_ world-id x y z]
      (boolean (bmc/farmland-block? (server-context/get-server) world-id x y z)))))
